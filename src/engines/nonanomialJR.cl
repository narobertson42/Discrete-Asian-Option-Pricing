...
__kernel void discreteMonteCarloKernel(__global float *callSums,
  ...
  async_work_group_copy(shifts, globalShifts, 256, 0);
  ...
  for (int j = 0; j < steps; j = j) {
    ...
    for (int i = 0; i < stepsLeft; i++) {
      ...
      currentValue = currentValue * shifts[random & 0b11111111];
      cumulativeWalk += currentValue;
      random = random >> 8;
      j++;
    }
...
__kernel void discreteMonteCarloKernelAntithetic(
  ...
  for (int j = 0; j < steps; j = j) {
    ...
    for (int i = 0; i < stepsLeft; i++) {
      ...
      currentValue = currentValue * shifts[random & 0b11111111];
      currentValue2 = currentValue2 * shifts[(0xffffffff - random & 0b11111111)];
      cumulativeWalk += currentValue;
      cumulativeWalk2 += currentValue2;
      random = random >> 8;
      j++;
